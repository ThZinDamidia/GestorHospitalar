...
def atualizar_medico(id_medico, nome=None, data_nascimento=None, nacionalidade=None,
    ...
    if nacionalidade is not None: medico["nacionalidade"] = nacionalidade
    if especialidade is not None: medico["especialidade"] = especialidade
    ...
    if idiomas is not None: medico["idiomas"] = idiomas
    if ponto_forte is not None: medico["ponto_forte"] = ponto_forte
    if ponto_fraco is not None: medico["ponto_fraco"] = ponto_fraco
    if id_unidade is not None: medico["id_unidade"] = id_unidade
    if horario_turno is not None: medico["horario_turno"] = horario_turno
    if cargo is not None: medico["cargo"] = cargo
